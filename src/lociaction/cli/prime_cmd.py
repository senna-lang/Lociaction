...
BEGIN_MARKER = "<!-- BEGIN LOCIACTION -->"
END_MARKER = "<!-- END LOCIACTION -->"

PRIME_TEXT = """\
## Past Memory Search (lociaction)

lociaction records every past conversation turn, decision, and code location. Retrieve that memory **before** acting — not after.
...
Pass the selection straight through — you do not need to find the enclosing function yourself first, lociaction resolves it for you:
...
def prime() -> None:
    ...
    if not (root / LOCIACTION_DIR).exists():
        return
    typer.echo(PRIME_TEXT)


def inject_agents_md(project_root: Path) -> bool:
    """Insert or update the lociaction section in the common AGENTS.md.
