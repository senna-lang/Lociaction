...
- 共有 JSON 設定ファイルへのマージ: claude（`lociaction.hooks` の専用実装）,
...
from lociaction.adapters.harness.lifecycle import LifecycleCommands, lifecycle_commands
from lociaction.config import DEFAULT_DISTILL_BATCH_LIMIT
from lociaction.core.ports import Hooks
...
class GrokHooks:
    ...
    lociaction 専用の `lociaction.json` 1本に `MergedJsonHookWriter` で書く。
    ...
    def _settings_path(self) -> Path:
        return Path.home() / ".grok" / "hooks" / "lociaction.json"
...
def _dedicated_file_header(harness_note: str) -> str:
    from lociaction.adapters.harness.hook_writers import DEDICATED_FILE_MARKER

    return (
        "// installed by lociaction\n"
        "// managed by lociaction; reinstalling overwrites this file.\n"
...
def _render_omp_pi_extension(commands: LifecycleCommands) -> str:
    """`~/.omp/agent/extensions/lociaction.ts` の中身を組み立てる。
...
def _render_opencode_plugin(commands: LifecycleCommands) -> str:
    """`~/.config/opencode/plugins/lociaction.ts` の中身を組み立てる。
    ...
    return (
        ...
        f"{_run_js_snippet()}\n"
        "\n"
        "export const LociactionHook = async ({ directory }) => ({\n"
        "\tevent: async ({ event }) => {\n"
        "\t\tswitch (event?.type) {\n"
        '\t\t\tcase "session.idle":\n'
...
class OmpPiHooks:
    ...
    def _target_path(self) -> Path:
        return Path.home() / ".omp" / "agent" / "extensions" / "lociaction.ts"
...
class OpenCodeHooks:
    ...
    def _target_path(self) -> Path:
        return Path.home() / ".config" / "opencode" / "plugins" / "lociaction.ts"
...
class FallbackHooks:
    ...
    def uninstall(self) -> tuple[bool, str]:
        return (
            False,
            f"{self._harness} has no native lociaction hooks to uninstall.",
        )
