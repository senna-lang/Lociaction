...
ライフサイクル:
  ...
  - ソケットファイルは .lociaction/embedder.sock
...
from lociaction.paths import PID_FILENAME
...
def _load_embedder() -> Embedder:
    ...
    os.environ["LOCIACTION_NO_SOCK"] = "1"
    from lociaction.embedder import Embedder

    embedder = Embedder()
    del os.environ["LOCIACTION_NO_SOCK"]
    return embedder
...
def main() -> None:
    """CLI エントリポイント: python -m lociaction.embedder_server <sock_path>"""
    if len(sys.argv) < 2:
        print("Usage: python -m lociaction.embedder_server <sock_path>", file=sys.stderr)
        sys.exit(1)
